...
```{r}
value_box(
  title = "Mean number of items sold by a shop",
  value = mean_items_sold,
  theme = "teal"
)
```
...
```{r}
value_box(
  title = "Total items sold overall",
  value = 1324,
  theme = "teal"
)
```
...
value_box(
  ...
  value = "Cloak of the Undead King: 501 items sold",
  showcase = tags$img(src = "images/cloak_of_the_undead_king.png", height = "60px"),
  theme = "teal"
)
```
...
output$lollipop <- renderPlot({
  futuretech_filtered() |> 
  group_by(product) |>
  summarize(total_product_sold = sum(units_sold), image_url = image_url) |> 
  ggplot(aes(x = reorder(product, total_product_sold)), y = total_product_sold) +
    geom_segment(aes(xend = product, y = 0, yend = total_product_sold), color = "gray") +
